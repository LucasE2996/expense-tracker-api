-- as we want a new DB each time this run we have first do drop any existent DB/user with the same name
drop database expensetrackerdb;
drop user expensetracker;

--usually we don't use sysadmin user for aplications so let's create a new user
create user expensetracker with password 'password';

-- now we need to create our database. template0 is provided by prostgres
